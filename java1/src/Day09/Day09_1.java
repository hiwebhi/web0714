...
public class Day09_1 {
	...
	//클래스
		...
		//접근제한자: 
			...
			//public: 프로젝트 내 모든곳에서 호출 가능[사용가능]
			//private: 현재 클래스 내에서만 호출 가능
			//protected: 현재 패키지 내에서만 호출 가능 [단, 상속받은 서브클래스는 슈퍼클래스가 있는 패키지로 접근가능]
			//default: 현재 패키지 내에서만 호출 가능
		
		//자주 사용되는 키워드[미리 정의된 단어]
			//final: 메모리의 상수[고정값]   (변수<->상수)
			//static: 정적변수[고정변수]
				//프로그램 시작과 동시에 메모리 할당 =>static
				//프로그램 종료와 동시에 메모리 초기화
					//목적:프로그램 전반적인 곳애 사용되는 메모리 할당시
	...
	public static void main(String[] args) {
		...
		//게임속의 캐릭터==객체
			...
			System.out.println("1.final이 없을경우");
			character.lv=2; //변경가능
			System.out.println("2.final이 있을경우"); //변경불가
			//character.code="30";
	}
}
