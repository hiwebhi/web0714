	...
	public static void main(String[] args) {
		...
		while(true) {//자판기 무한루프[종료조건 없음]
			...
			while(true) { //사람마다 무한루프[while{ } : 괄호안 코드가 무한실행]
				                  //종료조건[결제했을경우(4): break; : 반복문탈출(가장가까운 반복문 괄호 나가기)
			System.out.println("\n[[[[자판기]]]]");
			...
			if(선택 ==4) { 
				...
				if(콜라구매수==0 && 환타구매수==0 && 사이다구매수==0) {System.err.println("[[[구매하신 제품이 없습니다 ]]]"); break;}
				else {
				  if(콜라구매수 !=0)System.out.println("콜라 구매수"+콜라구매수+"결재액:"+콜라구매수*300);
				  if(환타구매수 !=0)System.out.println("환타 구매수"+환타구매수+"결재액:"+환타구매수*200);
				  if(사이다구매수 !=0) System.out.println("사이다 구매수"+사이다구매수+"결재액:"+사이다구매수*100);
				  
				  총결제액=(콜라구매수*300)+(환타구매수*200)+(사이다구매수*100);
				  
				  System.out.println("결재 하시겠습니까? 예[1] 아니요[0]"); int 선택2=scanner.nextInt();
				  if(선택2==1) {
					System.out.println("[[[금액 투여]]]:");int 금액=scanner.nextInt();
					//1.총결제액보다 금액이 적은경우 결제x
					if (금액 < 총결제액) {
						System.out.println("[[[금액부족(초기화면)]]]");
						콜라재고+=콜라구매수;
						사이다재고+=사이다구매수;
						환타재고+=환타구매수;
						break;
