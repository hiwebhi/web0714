...
public class LoginController implements Initializable{
	...
	public LoginController() { //현재 클래스의 메모리 받아 객체 생성하는 생성자
		instance=this;
	}
	...
	//초기 메소드 재정의 //fxml 파일이 열렸을때 초기 메소드
	@Override
    ...
    void login(MouseEvent event) { //로그인 버튼 클릭시
    	...
    	PauseTransition pauseTransition= new PauseTransition(); //객체생성
    	...
    	pauseTransition.setOnFinished(인수 -> {
    		...
    		if(result) {
    			...
    			try {
            		...
            		stage.setTitle("Diseney Store"); //스테이지 타이틀
            		//스테이지 아이콘
    				//1. 이미지 불러오기
    ...
    @FXML
    void signup(MouseEvent event) throws Exception{ //회원가입 눌렀을때 
    								//// throws:예외 던지기
    	//1.스테이지 생성
    	Stage stage=new Stage();
