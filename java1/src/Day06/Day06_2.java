...
public static void main(String[] args) {
	...
	int end=0;
	Integer[] 학생점수목록=null; //클래스로 int 선언
	while(true) {//무한루프 [종료조건:4번눌렀을때 break;]
			...
			switch(ch) {
			    case 1:
			    	System.out.print("학생수 입력 : "); int count=scanner.nextInt();
			    	 //int[] 학생점수목록 : 동일한 이름으로 메모리 선언은 한번만
				...
				case 3:
					...
					//4개 인덱스가 존재했을경우[0]:3변  [1]:2번  [2]:1번  [3]:0번
 					/*
					...
					Arrays.sort(학생점수목록);
					Arrays.sort(학생점수목록,Collections.reverseOrder()); //내림차순
					...
					for(int i=0; i<학생점수목록.length;i++) {
						...
						합계 +=학생점수목록[i]; //합계=합계+학생점수목록[i];
					}
					System.out.println("[[전체평균]]"+(합계/학생점수목록.length));
					break;
				case 4:
