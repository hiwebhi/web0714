...
public class SignupController implements Initializable{
    ...
    void signup(MouseEvent event) {
    	...
    	if(txtid.getText().length()<=3||txtid.getText().length()>10) {
    		lblconfirm.setText("아이디는 4~10글자 이내만 가능합니다.");
    		return;
    	}
    		//2.비밀번호와 비밀번호 확인이 서로 다른 경우 실패
    	...
    	if(txtpassword.getText().length()<8||txtpassword.getText().length()>15) {
    		lblconfirm.setText("비밀번호는 8~15자 이내만 가능합니다");
    		return;
    	}
    		//4.이메일 반드시 @ 포함되도록
		...
		*/
    	...
    	//회원가입성공[메세지 띄우고 해당 스테이지 닫기]
    	//1.메세지 객체 만들기 [Alert 클래스]
    	Alert alert=new Alert(AlertType.INFORMATION);
    	//2.메시지 내용 넣기
    	alert.setContentText("Diseney Store 가입을 축하합니다 [축하 포인트 100 지급]");
    	alert.setHeaderText("회원가입 성공");
    	//3.메시지 실행
    	alert.showAndWait(); //창열고 닫을때까지 기다리기
    	//4.현재 회원가입 스테이지 끄기
